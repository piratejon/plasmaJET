...
# game starts with empty board
    ...
    self.assertEqual(play.get_status(game_id), "2")
    self.assertEqual(play.decode_grid_json(play.get_grid(game_id)), ['O','X','','O','X','X','O','O',''])
    play.make_move(game_id, p2_id, 2)
    self.assertEqual(play.get_status(game_id), "1")
    self.assertEqual(play.decode_grid_json(play.get_grid(game_id)), ['O','X','O','O','X','X','O','O',''])
    play.make_move(game_id, p1_id, 5)
    self.assertEqual(play.get_status(game_id), "2")
    self.assertEqual(play.decode_grid_json(play.get_grid(game_id)), ['O','X','O','O','X',' ','O','O','X'])
    play.make_move(game_id, p1_id, 2)
    self.assertEqual(play.get_status(game_id), "1")
    self.assertEqual(play.decode_grid_json(play.get_grid(game_id)), ['O','X',' ','O','X','O','O','O','X'])
    play.make_move(game_id, p1_id, 1)
